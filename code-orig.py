...
def add_layer(RG):
    ...
    for v in RG.graph['chains_data']:
        assert(l == cd[v]["len"])
        stall = max([cd[w]["last_block" if cd[w]["len"] == l else "one_before_last_block"]
                     for w in cd[v]["neighbors"]]) + Exp()
        RG.add_node((v,stall))
        RG.add_edges_from([((v,stall),(w,cd[w]["last_block" if cd[w]["len"] == l else "one_before_last_block"])) for w in cd[v]["neighbors"]])
        cd[v]["one_before_last_block"] = cd[v]["last_block"]
        cd[v]["last_block"] = stall
        cd[v]["len"] += 1
